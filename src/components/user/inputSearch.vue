<template>
    <div class="user-search-wapper">
        <div action="">
            ...
            <ul v-show="isHidden">
                <li v-for="(product, index) in products" :key="product._id" @click="updateActiveIndex(index)"> <router-link
                        @click="handleClick" :to="'/allproduct/' + product._id">{{ product.name }}</router-link></li>
            </ul>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        //         try {
        //             const respone = await productService.findById(id);

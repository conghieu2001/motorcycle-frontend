...
<template>
  <div class="overlayy" v-if="activeFormChangeInfor">
    <div class="box-changeInfor-pass">
      <div class="d-flex justify-content-end align-items-baseline confirm-titlee">
        <div class="title-changeInfor">Chỉnh sửa thông tin cá nhân</div>
        <div class="me-3 background-close" @click="closeFormChangeInfor">
          <div>
            <svg class="icon-close" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 352 512">
      ...
      <form class="ps-4 pe-4" @submit.prevent.stop="updateUser" enctype="multipart/form-data">
        <div class="user-image d-flex mt-3">
          ...
          <div class="user-image-edit">
            <div>
              <img :src="'http://localhost:3000/' + inforUser2.avatar" />
              <img class="ms-5" v-if="showImg != ''" :src="showImg" alt="">
            </div>
        ...
        <div class="user-name d-flex mt-3 mb-3">
          <h5 class="">Tên</h5>
          <input class="ps-3" type="text" v-model="inforUser2.fullName" disabled />
        </div>
        <hr>
        <div class="user-email d-flex mt-3 mb-3">
          <h5>Email</h5>
          <input class="ps-3" type="email" v-model="inforUser2.email" disabled />
        </div>
        <hr>
        <div class="user-sdt d-flex mt-3 mb-3">
          <h5>SĐT</h5>
          <input class="ps-3" type="text" v-model="inforUser2.phoneNumber" disabled />
        </div>
        <hr>
